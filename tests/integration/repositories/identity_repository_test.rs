...
use domain::{
    entities::{customer::CreateCustomer, identity::CreateIdentity},
    repositories::{
...
async fn create_and_find_by_user(pool: PgPool) {
    ...
    repo.create(make_identity(user, "google", "ext-1"))
        .await
        .unwrap();
...
async fn find_by_user_excludes_deleted(pool: PgPool) {
    ...
    let identity = repo
        .create(make_identity(user, "google", "ext-del"))
        .await
        .unwrap();
...
async fn find_active_returns_existing(pool: PgPool) {
    ...
    repo.create(make_identity(user, "apple", "apple-ext"))
        .await
        .unwrap();
...
async fn find_active_returns_none_for_deleted(pool: PgPool) {
    ...
    let identity = repo
        .create(make_identity(user, "apple", "apple-ext"))
        .await
        .unwrap();
    repo.soft_delete(identity.id, user).await.unwrap();
...
async fn find_deleted_returns_soft_deleted(pool: PgPool) {
    ...
    let identity = repo
        .create(make_identity(user, "the1", "the1-ext"))
        .await
        .unwrap();
    repo.soft_delete(identity.id, user).await.unwrap();
...
async fn soft_delete_marks_deleted(pool: PgPool) {
    ...
    let identity = repo
        .create(make_identity(user, "fb", "fb-ext"))
        .await
        .unwrap();
...
async fn update_tokens_replaces_stored_tokens(pool: PgPool) {
    ...
    let identity = repo
        .create(make_identity(user, "the1", "the1-update"))
        .await
        .unwrap();
    let updated = repo
        .update_tokens(
    ...
    assert_eq!(
        updated.provider_refresh_token.as_deref(),
        Some("new-refresh")
    );
}
...
async fn log_transaction_inserts_audit_row(pool: PgPool) {
    ...
    repo.log_transaction(user, "delete", "google", "ext-audit")
        .await
        .unwrap();
