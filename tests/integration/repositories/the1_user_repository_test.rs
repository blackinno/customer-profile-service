...
fn upsert_data(member_id: &str, card_number: Option<&str>) -> UpsertThe1User {
    UpsertThe1User {
        ...
        tiers: vec![
            UpsertTier { code: "GOLD".to_string(), name: Some("Gold".to_string()), expired_date: None },
        ],
    }
}
...
async fn upsert_creates_new_the1_user(pool: PgPool) {
    ...
    let the1 = repo.upsert(user, upsert_data("MEM001", Some("1234567890123456"))).await.unwrap();
...
async fn upsert_updates_existing_the1_user(pool: PgPool) {
    ...
    repo.upsert(user, upsert_data("MEM001", Some("1111111111111111"))).await.unwrap();
    let updated = repo.upsert(user, upsert_data("MEM001", Some("2222222222222222"))).await.unwrap();
...
async fn find_by_user_returns_existing(pool: PgPool) {
    ...
    repo.upsert(user, upsert_data("MEM002", None)).await.unwrap();
...
async fn find_by_card_number_returns_existing(pool: PgPool) {
    ...
    repo.upsert(user, upsert_data("MEM003", Some("9999999999999999"))).await.unwrap();
...
async fn find_by_member_id_returns_existing(pool: PgPool) {
    ...
    repo.upsert(user, upsert_data("MEMBER-FIND", None)).await.unwrap();
